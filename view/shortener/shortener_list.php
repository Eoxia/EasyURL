...
/**
 * \file    view/shortener/shortener_list.php
 * \ingroup easyurl
 * \brief   List page for shortener
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'shortenerlist'; // To manage different context of search
...
$hookmanager->initHooks(['shortenerlist']); // Note that conf->hooks_modules contains array
...
$permissiontodelete = $user->rights->easyurl->shortener->delete;
...
if (empty($reshook)) {
    ...
    // Mass actions
    $objectclass = 'Shortener';
    $objectlabel = 'Shortener';
    $uploaddir   = $conf->easyurl->dir_output;
...
$title    = $langs->trans('ShortenerList');
...
$sql .= ' AND t.status >= ' . $object::STATUS_DRAFT;
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
    ...
    $id  = $obj->rowid;
    header('Location: ' .dol_buildpath('/easyurl/view/shortener/shortener_card.php', 1) . '?id=' . $id);
    exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/easyurl/view/easyurltools.php', 1), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER['PHP_SELF'], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbTotalOfRecords, 'object_' . $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = 'SendShortenerRef';
$modelmail = 'shortener';
$objecttmp = new Shortener($db);
    ...
    if ($mode == 'kanban') {
        ...
        foreach ($object->fields as $key => $val) {
            ...
            if (!empty($arrayfields['t.' . $key]['checked'])) {
                ...
                print '>';
                if ($key == 'status') {
                    print $object->getLibStatut($conf->browser->layout == 'phone' ? 3 : 5);
                } elseif ($key == 'fk_element') {
                    if (dol_strlen($object->element_type) > 0) {
                        $objectsMetadata = saturne_get_objects_metadata($object->element_type);
                        $val['type'] = 'integer:' . $objectsMetadata['class_name'] . ':' . $objectsMetadata['class_path'];
                        print $object->showOutputField($val, $key, $object->fk_element);
                    }
                } elseif ($key == 'rowid') {
